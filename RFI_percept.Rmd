...
#             lapse_rate: should functin fit lapse rate? TRUE or FALSE. Defaults to FALSE
...
psy_fun <- function(x_val, rsp, par, lapse = FALSE, invert = FALSE){
  ...
  if(length(par) == 3 & lapse_rate == FALSE) {height_ctrl <- par[3]}
  if(length(par) == 3 & lapse_rate == TRUE) {lapse_ctrl <- par[3]}
